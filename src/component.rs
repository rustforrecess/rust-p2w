...
fn export_sig(
    ...
    let result = match return_type {
        ...
        Some(_) => match wit_ty(return_type) {
            ...
            None => {
                return Err(
                    "the return annotation must be int / float / str / list[...]".to_string(),
                );
            }
        },
    };
...
/// The reverse of `list_elem_expr`: how a `lower_list_of_*` result builder
/// writes ONE p2w element `e` into the canonical buffer at index `i`.
/// Returns `(buffer C type, element stride bytes, alignment, write stmt)`.
/// Only scalar elements — flat list returns (nested returns are refused).
fn lower_list_elem(elem: &WitType) -> (&'static str, usize, usize, String) {
    match elem {
        WitType::S32 => ("int", 4, 4, "buf[i] = p2w_unbox_int(e);".to_string()),
        WitType::F64 => ("double", 8, 8, "buf[i] = p2w_unbox_float(e);".to_string()),
        // A string element is a canonical (ptr, len) pair pointing at the p2w
        // string's bytes — which stay valid because the owning list is only
        // freed in cabi_post, after the host has copied the result out.
        WitType::Str => (
            "int",
            8,
            4,
...
fn shim_c(exports: &[WitExport], imports: &[&'static str]) -> String {
    let mut c = String::from(
        "/* generated by rust-p2w's component converter (LESSON_PLAYER.md step 5e) */\n\
         ...
         extern double p2w_unbox_float(int v);\n\
         ...
         extern int p2w_len(int v);\n\
         extern int p2w_index(int list, int i);\n\
    ...
    for lt in &result_lists {
        let WitType::List(elem) = lt else { continue };
        let (buf_ty, stride, align, write) = lower_list_elem(elem);
        c.push_str(&format!(
            "static int lower_{}(int list, int* len_out) {{\n  \
               int len = p2w_unbox_int(p2w_len(list));\n  *len_out = len;\n  \
               {buf_ty}* buf = ({buf_ty}*)cabi_realloc(0, 0, {align}, len * {stride});\n  \
               for (int i = 0; i < len; i++) {{\n    int e = p2w_index(list, p2w_int(i));\n    {write}\n    p2w_release(e);\n  }}\n  \
               return (int)buf;\n}}\n\n",
            lt.c_frag(),
        ));
    }
    ...
    for x in exports {
        ...
        match &x.result {
            ...
            // A (flat) list result: lower the p2w list into a canonical
            // (ptr, len) buffer via the generated lower_* helper, write it to
            // the return area, and free the list in cabi_post (string elements
            // point into it, so it must outlive the host's copy).
            Some(WitType::List(_)) => {
                let frag = x.result.as_ref().unwrap().c_frag();
                c.push_str(&format!(
                    "static int {api}_ret[2];\nstatic int {api}_saved;\n\
                     __attribute__((export_name(\"{kebab}\")))\n\
                     int x_{api}({sig}) {{\n  cabi_off = 0;\n{pre}  int v = {call};\n{post}  \
                     {api}_saved = v;\n  int {api}_len;\n  \
                     {api}_ret[0] = lower_{frag}(v, &{api}_len);\n  \
                     {api}_ret[1] = {api}_len;\n  return (int){api}_ret;\n}}\n\
                     __attribute__((export_name(\"cabi_post_{kebab}\")))\n\
                     void cabi_post_x_{api}(int ptr) {{ (void)ptr; p2w_release({api}_saved); }}\n\n",
                    api = x.api_name,
...
mod tests {
    ...
    #[test]
    fn flat_list_returns_lower_to_canonical_buffers() {
        // A component that COMPUTES and returns a list — the compute-and-report
        // class extended to sequences.
        let src = "def stats_summary(a: int, b: int, c: int) -> list[int]:\n    return [a, b, c]\n";
        let x = to_component(src, "stats", &api(&["summary"])).expect("convert");
        assert!(
            x.wit
                .contains("export summary: func(a: s32, b: s32, c: s32) -> list<s32>;"),
            "{}",
            x.wit
        );
        // A lower_* builder reads the p2w list into a canonical (ptr, len) buffer…
        assert!(
            x.shim_c
                .contains("static int lower_list_of_s32(int list, int* len_out)"),
            "{}",
            x.shim_c
        );
        assert!(
            x.shim_c.contains("buf[i] = p2w_unbox_int(e);"),
            "{}",
            x.shim_c
        );
        // …the wrapper writes it to the return area and frees the list post-copy.
        assert!(
            x.shim_c.contains("lower_list_of_s32(v, &summary_len);"),
            "{}",
            x.shim_c
        );
        assert!(
            x.shim_c.contains("export_name(\"cabi_post_summary\")"),
            "{}",
            x.shim_c
        );

        // list<string> returns: elements point into the still-owned list.
        let s = "def tags_all() -> list[str]:\n    return [\"a\", \"b\"]\n";
        let x = to_component(s, "tags", &api(&["all"])).expect("convert");
        assert!(
            x.wit.contains("export all: func() -> list<string>;"),
            "{}",
            x.wit
        );
        assert!(
            x.shim_c
                .contains("buf[i * 2] = p2w_str_ptr(e); buf[i * 2 + 1] = p2w_str_len(e);"),
            "{}",
            x.shim_c
        );
    }
